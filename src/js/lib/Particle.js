...
function TextParticle(selector) {
	...
	function createParticle() {
		...
		particle.pixiCircle.scale.x = 0.12
		particle.pixiCircle.scale.y = 0.12
		...
		particle.move = function () {
			...
			if (this.dSqr < MIN_DIST_SQR) {
				...
				this.a = Math.atan2(this.dy / this.dy > 0 ? this.dy : -this.dy, this.dx / this.dx > 0 ? this.dx : -this.dx);
			...
			for (var j = 0; j < imageData.height; j += skipCount) {
				...
				if (color == 255) {
					...
					particles[particles.length - 1].initPosition(
						...
						j + ((app.view.height / 2) - (imageData.height / 2)) - imageData.height / 2);
				}
			}
		}
